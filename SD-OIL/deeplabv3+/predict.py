...
#     #----------------------------------------------------------------------------------------------------------#
#     test_interval = 100
#     fps_image_path  = "img/25_training.jpg"
#     #-------------------------------------------------------------------------#
...
# 设置路径和参数
image_path = "VOCdevkit/VOC2007/JPEGImages/000018.jpg"
gt_path = "VOCdevkit/VOC2007/SegmentationClass/000018.png"
# image_path = "img/000182.jpg"  # 替换为您要预测的图像路径
# gt_path = "img/000182.png" # 替换为您要预测的图像对应的真实标签路径
num_classes = 2
name_classes = ["background", "Oil"]
...
# 将预测图像和真实标签转换为numpy数组
pred = np.array(pred_image)
label = np.array(Image.open(gt_path))

# 保存预测结果（可选）
if pred.max() <= 1.0:
    pred_result = (pred * 255).astype(np.uint8)
pred_result = Image.fromarray(pred_result)

pred_result.save("pred_result_000018.png")  # 替换为保存路径
...
for i in range(label.shape[0]):
    for j in range(label.shape[1]):
        if label[i, j] < num_classes:
            hist[label[i, j], pred[i, j]] += 1
